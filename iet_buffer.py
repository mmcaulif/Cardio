...
class IeTable:
	def __init__(self, network, capacity=1_000_000, transition_func=TorchTransition, k_events=4, cluster_interval=10, percent=0.5):
	...
	def sample(self, batch_size):
		...
		from_buffer += int(batch_size-(from_tables*self.k_events))	# ensure batch_size samples are received

		s_batch, a_batch, r_batch, sp_batch, d_batch, _ = self.buffer.sample(from_buffer)
