...
for steps in range(20_000):
    ...
    if steps % 2 == 0:
        ...
        (
            updates,
            a_opt_state,
        ) = a_optimizer.update(a_grads, a_opt_state)
        ...
        def policy_loss(params, q_params, x):
            ...
            return -jnp.mean(
                critic.apply(q_params, x, a_sampled) - (ent_coeff * dist.entropy())
            )

    # see if you can do this more smoothly
    targ_c_params = jax.tree_util.tree_map(
        lambda x, y: (1 - 0.005) * x + 0.005 * y, targ_c_params, c_params
    )
