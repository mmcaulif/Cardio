...
class CEM:
    ...
    def sample(self, n):
        theta_list = (
            self.mean
            + th.normal(
                mean=th.zeros([n, self.theta_dim]), std=th.zeros([n, self.theta_dim])
            )
            * self.std
        )
    ...
    def update(self, selected_samples):
        ...
        )  # plus the entropy offset, or else easily get 0 std
        # print('std: ', self.std)
...
runner = Runner(
    ...
    collector=Gatherer(
        env=env,
        warmup_len=10_000,
        logger_kwargs=dict(log_dir=env_name, log_interval=2_500),
    ),
    backend="pytorch",
)
...
for steps in range(150_000):
    ...
    with th.no_grad():
        ...
        a_p = np.array(
            [
                runner.policy.cem_optimal_action(next_state, targ_critic)
                for next_state in s_p
            ]
        )
