...
def main():
	...
	for t in range(10000):
		...
		with th.no_grad():
			q_p = th.max(targ_net(s_p, th.zeros(1, runner.batch_size, 64), only_q=True), keepdim=True, dim=-1).values.transpose(1,2)
			y = r + gamma * q_p * (1 - d)

		q = critic(s, th.zeros(1, runner.batch_size, 64), only_q=True).gather(2, a.long())
