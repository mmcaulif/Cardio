...
class Policy(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.alpha = nn.Sequential(nn.Linear(300, action_dim), nn.Softplus())

        self.beta = nn.Sequential(nn.Linear(300, action_dim), nn.Softplus())
...
c_optimizer = th.optim.Adam(
    critic.parameters(), lr=7.3e-4
)  # , weight_decay=2.4e-4)	# weight_decay value from Relu to the rescue paper
a_optimizer = th.optim.Adam(
    actor.parameters(), lr=7.3e-4, weight_decay=2.4e-4
)  # weight_decay value from Relu to the rescue paper
...
ent_optim = th.optim.Adam(
    [log_alpha], lr=7.3e-4
)  # check if lr is same as critic and actor
H = -4
tau = 0.02
...
for steps in range(4_532):
    ...
    for batch in n_batches:
        ...
        if steps % 2 == 0:
            ...
            awr_loss = (
                dist.log_prob(a.squeeze(1)).sum(-1, keepdim=True) * weight
            ) * 0.05

            policy_loss = -(
                th.min(*critic(s, a_sampled)) - (ent_coeff * log_prob) + awr_loss
            ).mean()
