...
def torch_transition(s, a, r, s_p, d):
    s = th.from_numpy(np.array(s)).float()
    a = th.from_numpy(np.array(a)).float()
    r = th.from_numpy(np.array(r)).unsqueeze(1).float()
    s_p = th.from_numpy(np.array(s_p)).float()
    d = th.from_numpy(np.array(d)).unsqueeze(1).int()
    return s, a, r, s_p, d
